...
// Maximum bytes to fetch when checking for moov atom
...
export async function GET(request: NextRequest) {
  ...
  try {
    if (source === "local") {
      // For local files, we'd need ffprobe - return a note for now
      return NextResponse.json({
        success: true,
        source: "local",
        results: [],
        summary: { total: 0, faststart: 0, needsOptimization: 0, unknown: 0 },
        error: "Local file faststart checking requires ffprobe. Use remote source or check files manually with: ffprobe -v quiet -show_format <file> | grep -i faststart",
      });
    }
